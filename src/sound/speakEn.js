...
function deferSpeechStart(synth, warmupVoice, prepare, start) {
    ...
    const warmThenSchedule = () => {
        ...
        try {
            ...
            warmup = createUtterance(".", warmupVoice.locale, warmupVoice.characterId, warmupVoice.brightJapanese, warmupVoice.avoidVoiceCharacterId);
...
function createUtterance(text, lang, characterId, brightJapanese = false, avoidVoiceCharacterId) {
    ...
    const femaleEnglishVoice = englishVoices.find((voice) => /aria|jenny|zira|samantha|victoria|karen|moira|tessa|female/i.test(voice.name));
...
export function speakEn(text, onEnd, lang = "en", onStart, onBoundary) {
...
export function speakSpeechQueue(items, callbacks, characterId) {
    ...
    const disposeStart = deferSpeechStart(synth, {
        ...
        try {
            queue.forEach((item, index) => {
                ...
                utterance.onstart = () => {
                    if (stopped || ended.has(index))
                        return;
                    activeIndex = index;
                    callbacks.onItemStart(item, index);
                };
                utterance.onboundary = (event) => {
                    if (!stopped && activeIndex === index && !ended.has(index))
